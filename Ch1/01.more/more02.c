 ...
 * feature of version0.2: reads from /dev/tty for commands
 */
#include <stdio.h>
#include <stdlib.h>
#define PAGELEN 24
...
void do_more(FILE *fp)
{
	...
	int num_of_line =
		0; // record how many lines we have read relatively, which ranges from  0 to 24
	...
	while (NULL != fgets(line, LINELEN, fp)) {
		if (num_of_line == PAGELEN) {
			reply = see_more(
				fp_tty); // NEW: directly read from keyboard
			if (reply == 0) {
				break;
			}
			num_of_line -= reply;
...
int see_more(FILE *cmd)
{
	...
	while ((c = getc(cmd)) != EOF) { // NEW: read directly from keyboard
		/* printf("deug: c=%c\n", c); */
		if (c == 'q') {
			return 0;
		}
