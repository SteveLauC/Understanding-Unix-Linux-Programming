...
static mut seg_id: i32 = 0;
static mut sem_set_id: i32 = 0;

fn main() {
    unsafe {
        signal(Signal::SIGINT, SigHandler::Handler(clean_up)).unwrap();
    ...
    unsafe {
        sem_set_id = semget(TIME_SEM_KEY, 2, IPC_CREAT | IPC_EXCL | 0o666);
...
fn set_sem_value(semset_id: i32, sem_num: i32, val: i32) {
    if unsafe { semctl(semset_id, sem_num, SETVAL, val) } == -1 {
        eprintln!("semctl");
        exit(4);
    }
}
...
fn wait_and_lock(semset_id: i32) {
    ...
    if unsafe { semop(semset_id, &mut actions as *mut sembuf, 2) } == -1 {
...
fn release_lock(semset_id: i32) {
    ...
    if unsafe { semop(semset_id, &mut actions as *mut sembuf, 1) } == -1 {
        eprintln!("semop: unlocking");
        exit(10);
    }
}

extern "C" fn clean_up(_signum: i32) {
    unsafe {
        shmctl(seg_id, IPC_RMID, null() as *const shmid_ds as *mut shmid_ds);
        semctl(sem_set_id, 0, IPC_RMID, null::<*const i32>());
    }
