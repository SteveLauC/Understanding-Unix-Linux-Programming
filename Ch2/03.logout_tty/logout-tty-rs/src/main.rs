...
fn logout_tty(line: &str) -> Result<(), LogoutError> {
	...
	let buf: &mut [u8] = unsafe{slice::from_raw_parts_mut(&rec as *const utmpx as *const u8 as *mut u8, len)};
	...
	while utmp_file.read(buf).is_ok() {
		if unsafe{CStr::from_ptr(rec.ut_line.as_ptr()).to_str().unwrap()} == line {
			...
			rec.ut_tv.tv_sec = SystemTime::now().duration_since(UNIX_EPOCH).expect("can not calculate time elapsed").as_secs() as i32;
			rec.ut_tv.tv_usec = 0;
